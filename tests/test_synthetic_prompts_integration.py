...
""")
    ...
    def mock_subprocess_run(cmd, **kwargs):
        ...
        result.stdout = json.dumps({
            ...
            "audio_path": f"{tmp_path}/synthetic_prompts/{cmd[cmd.index('--speaker-id') + 1]}_seed_12345.wav",
            "stdout_transcript": f"[{cmd[cmd.index('--speaker-id') + 1]}] Dia is a state-of-the-art text-to-speech system designed for high-quality audio generation.  This system can produce natural-sounding speech that captures the nuances of human voice patterns.  The technology behind it represents significant advances in neural audio synthesis, making it particularly effective for creating realistic voice clones from minimal input data."
        })
        result.stderr = ""
        return result
    ...
    monkeypatch.setattr("src.pipeline.config.GENERATE_PROMPT_OUTPUT_DIR", str(tmp_path / "synthetic_prompts"))
...
""")
    ...
    def mock_subprocess_run(cmd, **kwargs):
        ...
        return MagicMock(returncode=0, stdout='{"audio_path": "", "stdout_transcript": ""}', stderr="")
    ...
    class MockDiaTTS:
        ...
        def generate(self, texts, unified_audio_prompt, unified_transcript_prompt):
            ...
            for _ in texts:
                ...
                segments.append(AudioSegment(data.tobytes(), frame_rate=22050, sample_width=2, channels=1))
            return segments
...
""")
    ...
    # Verify no subprocess calls were made (single chunk doesn't trigger synthetic prompts)
    assert len(subprocess_calls) == 0
    assert output_path.exists()
...
""")
    ...
    def mock_subprocess_run(cmd, **kwargs):
        ...
        result.stdout = json.dumps({
            "speaker_id": cmd[cmd.index("--speaker-id") + 1],
            "audio_path": f"{tmp_path}/synthetic_prompts/{cmd[cmd.index('--speaker-id') + 1]}_seed_12345.wav",
            "stdout_transcript": f"[{cmd[cmd.index('--speaker-id') + 1]}] Synthetic transcript"
        })
        result.stderr = ""
        return result
...
""")
    ...
    class MockDiaTTS:
        ...
        def generate(self, *args, **kwargs):
            ...
            import numpy as np
            data = np.zeros(int(0.1 * 22050), dtype=np.int16)
            return [AudioSegment(data.tobytes(), frame_rate=22050, sample_width=2, channels=1)]
...
""")
    ...
    def mock_subprocess_run(cmd, **kwargs):
        ...
        result.stdout = json.dumps({
            "speaker_id": cmd[cmd.index("--speaker-id") + 1],
            "audio_path": f"{tmp_path}/synthetic_prompts/{cmd[cmd.index('--speaker-id') + 1]}_seed_99999.wav",
            "stdout_transcript": f"[{cmd[cmd.index('--speaker-id') + 1]}] Synthetic transcript"
        })
        result.stderr = ""
        return result
...
""")
    ...
    def mock_subprocess_run(cmd, **kwargs):
        ...
        result.stdout = json.dumps({
            "speaker_id": cmd[cmd.index("--speaker-id") + 1],
            "audio_path": f"{synthetic_prompts_dir}/{cmd[cmd.index('--speaker-id') + 1]}_seed_12345.wav",
            "stdout_transcript": f"[{cmd[cmd.index('--speaker-id') + 1]}] Synthetic transcript"
        })
        result.stderr = ""
        return result
    ...
    monkeypatch.setattr("src.components.verbal_tag_injector.actor.Actor.perform_moment", mock_perform_moment)
    monkeypatch.setattr("src.pipeline.config.GENERATE_SYNTHETIC_PROMPTS", True)
    monkeypatch.setattr("src.pipeline.config.GENERATE_PROMPT_OUTPUT_DIR", str(synthetic_prompts_dir))
